...
async function parseEmblem(emblem, global_class_folder, local_class_folder) {
  ...
  await download(class_image_url, path.join(global_class_folder, img_name), function(error) {
    console.log(error);
  });
...
async function parseSpell(spell, global_class_folder, local_class_folder) {
  ...
  await download(spell_img_url, path.join(global_class_folder, img_name), function(error) {
    console.log(error);
  });
...
async function parseSpecialty(char_spec, global_class_folder, local_class_folder) {
  ...
  await download(spec_image_url, path.join(global_class_folder, img_name), function(error) {
    console.log(error);
  });

  return {
    passive : spec_info[0].replace('Passive: ', ''),
    active_name: active_name,
    active_info: spec_info[1].replace(active_name + ' ', ''),
    image: path.join(local_class_folder, img_name)
  };
}

module.exports.get_classes = async function (classes, public_dir_path, lib_path) {
  ...
  for (let c = 0; c < classes.length; c++) {
    await rp(classes[c].url).then(async function(html) {
      ...
      json_object.classes[classes[c].name].description = class_description;
      ...
      for (let key in characters) {
        ...
        await download(char_image_url, path.join(global_class_folder, img_name), function(error) {
          console.log(error);
        });
